...
with settings_col2:
    ...
    st.markdown('<p class="settings-desc">Shiprocket API credentials and status</p>', unsafe_allow_html=True)
    
    # Status indicator
    st.markdown('<span class="api-status connected">🟢 Connected to Shiprocket</span>', unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    
    st.text_input("📧 Email", value="openclawd12@gmail.com", disabled=True)
    st.text_input("🔑 API Token", value="••••••••••••••••••••••••", type="password", disabled=True)
    
    token_col1, token_col2 = st.columns(2)
    with token_col1:
        if st.button("🔄 Refresh Token", use_container_width=True):
            st.toast("Token refreshed successfully!", icon="✅")
    with token_col2:
    ...
    st.caption("Token expires in 9 days • Last refreshed: Today 8:30 AM")
    st.markdown('</div>', unsafe_allow_html=True)
